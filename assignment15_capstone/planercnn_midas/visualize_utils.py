...
def visualizeBatchDetection(options, config, input_dict, detection_dict, indexOffset=0, prefix='', suffix='', prediction_suffix='', write_ply=False, write_new_view=False):
    ...
    midas_depth = detection_dict["midas_depth"]
    print(midas_depth.shape)
    midas_depth_otpt = midas_depth.cpu().numpy()
    midas_depth_otpt = ((midas_depth_otpt / midas_depth_otpt.max()) * 255).astype(int)
    midas_depth_otpt = midas_depth_otpt.reshape((midas_depth.shape[1], midas_depth.shape[2], midas_depth.shape[0]))
    print(midas_depth_otpt)
    # mdf = midas_depth_otpt.reshape((midas_depth.shape[1], midas_depth.shape[2]))
    cv2.imwrite(options.test_dir + '/' + str(indexOffset) + '_midasDepth' + suffix + '.png', midas_depth_otpt)

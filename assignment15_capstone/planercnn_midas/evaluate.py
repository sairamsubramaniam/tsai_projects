...
from models.all_in_one import *
from models.load_midas_weights import load_midas_weights
...
class PlaneRCNNDetector():
    def __init__(self, options, config, modelType, checkpoint_dir=''):
        ...
        if not separate:
            if options.startEpoch >= 0:
                self.model.load_state_dict(torch.load(checkpoint_dir + '/checkpoint_' + str(options.startEpoch) + '.pth'), 
                                           strict=False )
                self.model = load_midas_weights(self.model)
            else:
        ...
        if 'refine' in modelType or 'final' in modelType:
            ...
            if not separate:
                ...
                self.model.load_state_dict(torch.load('checkpoint/pair_' + options.anchorType + '_pair/checkpoint.pth'), strict=False)
                self.model = load_midas_weights(self.model)
    ...
    def detect(self, sample):
        ...
        for indexOffset in [0, ]:
            ...
            rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, target_parameters, mrcnn_parameters, detections, detection_masks, detection_gt_parameters, detection_gt_masks, rpn_rois, roi_features, roi_indices, midas_depth, depth_np_pred = self.model.predict([images, image_metas, gt_class_ids, gt_boxes, gt_masks, gt_parameters, camera], mode='inference_detection', use_nms=2, use_refinement=True)
            ...
            detection_pair.append({'XYZ': XYZ_pred, 'depth': XYZ_pred[1:2], 'mask': detection_mask, 'detection': detections, 'masks': detection_masks, 'depth_np': depth_np_pred, 'plane_XYZ': plane_XYZ, "midas_depth": midas_depth})
            continue

        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            if 'inference' not in options.dataset:
                ...
                #for c in range(len(detection_pair)):
                    #np.save(options.test_dir + '/' + str(sampleIndex % 5000) + '_plane_parameters_' + str(c) + '.npy', detection_pair[c]['detection'][:, 6:9])
                    #np.save(options.test_dir + '/' + str(sampleIndex % 5000) + '_plane_masks_' + str(c) + '.npy', detection_pair[c]['masks'][:, 80:560])
                    #continue
                pass
                            
            if sampleIndex < 5000 or options.debug or options.dataset != '':
